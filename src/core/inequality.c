...
Interval solve_ineq_in(const int con, const int coeff, const Interval intvl) {
    ...
    const Interval intvl_greater =
        solve_ineq(con, coeff, intvl.left_open ? GREATER : GREATER_EQ, (int)intvl.low);
    const Interval intvl_less =
        solve_ineq(con, coeff, intvl.right_open ? LESS : LESS_EQ, (int)intvl.high);

    return intersection(intvl_greater, intvl_less);
}

Interval solve_ineq_sys(const int x_con, const int x_coeff, const int y_con, const int y_coeff,
                        const Interval xi, const Interval yi) {
    return intersection(solve_ineq_in(x_con, x_coeff, xi), solve_ineq_in(y_con, y_coeff, yi));
}
